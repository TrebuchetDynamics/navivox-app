...
| Deferred Hermes Desktop parity | Multi-endpoint/profile management is available locally with safe select/remove/disconnect confirmation; jobs inventory and bounded diagnostics are read-only; config/admin, memory UI, jobs admin, gateways, persona/SOUL, attachments/media, files/context folders, and raw diagnostics/log export are deferred. | Partially covered; remaining surfaces deferred/read-only by policy. |
