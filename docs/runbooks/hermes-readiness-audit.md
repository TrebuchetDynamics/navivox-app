...
| Multi-endpoint/profile management | `HermesEndpointStore`/`SecureHermesEndpointStore` now support saved endpoint profiles; the connect form renders selectable/renamable/deletable profile chips and keeps per-profile API keys in secure storage. | Covered locally. |
