...
| Messaging gateways, persona/SOUL, attachments, files/context folders | Surface readiness marks each as deferred with explicit copy; the surface-readiness dialog covers messaging gateways/persona without mutation controls, and the chat composer exposes safe copyable status sheets for attachments/media and files/context folders without upload controls. | Covered as deferred, not implemented. |
...
| Overall Dart/widget regression | `flutter test --coverage --concurrency=1` passed on 2026-07-17 with 543 tests and 73% line coverage (6,222 of 8,515 lines; 73.07% exact) using an isolated `/var/tmp` temporary directory. | Covered locally, but does not replace device/host receipts. |
...
| Documentation and local validation | README/changelog/runbook changes exist; the current 543-test serial coverage suite at 73.07%, formatting, static analysis, package contracts, and Maestro syntax checks have local receipts above. | Covered locally; this is not Android behavior evidence. |
...
2. **Deferred product surfaces** — config editing/admin, Hermes memory UI,
   ...
   Hermes mobile MVP. Unified contacts across saved Hermes endpoints and
   profiles, with one active streaming channel and gateway-scoped management,
   are now implemented locally.
