...
**Wing disposition:** Wing now provides filtered, tap-operable client-owned `/new`, `/sessions`, `/clear`, `/settings`, `/usage`, `/help`, `/agents`, `/providers`, `/tools`, `/schedules`, and `/gateway` suggestions, including 200% text-scale coverage. Exact local commands execute as client actions without sending slash text as an agent turn and cannot bypass an active run; unknown slash commands remain server-owned messages. Runtime model/agent command discovery is still **contract-gated** rather than reimplemented from guessed semantics.
