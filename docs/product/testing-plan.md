...
| Android voice path | `npm run android:voice-smoke`; `npm run android:hermes-voice-loop-smoke` -> `build/receipts/android-hermes-voice-loop-smoke.json`; required physical closeout uses `npm run android:live-mic-prep` + `../runbooks/android/live-mic-smoke.md` | Automated readiness uses deterministic transcript capture plus fake TTS/re-arm and must not claim physical mic evidence. Real spoken physical audio remains a separate required manual receipt before active-goal completion. Not whole-goal completion evidence by itself; run strict readiness audit before completion claims. |
...
| Completion blocker audit | `npm run hermes:readiness-audit`; `NAVIVOX_FAIL_ON_BLOCKERS=1 npm run hermes:readiness-audit` | Informational only; strict mode must fail while blockers remain. The helper must print `Completion verdict: NOT COMPLETE` with category-specific Android physical-mic, Hermes server-audio, deferred-surface, and/or receipt blockers, and must not promote proxy evidence such as tests, APK hashes, configured Hermes home, workflow YAML, dispatch-only output, or deterministic voice-loop receipts beyond their stated coverage. |
