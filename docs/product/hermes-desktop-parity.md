...
| Chat and runs | multi-run chat, streaming, reasoning, tools, approvals, clarify, queue, stop/retry/undo | concurrent session-owned streams, switching, durable detached-run duplicate protection/reconciliation, queue/retry, and bounded run token usage implemented; clarify, undo, notifications, and advanced metadata remain partial |
...
| 2. Providers and Models | `implementing` | Scoped provider-credential (presence/set/remove/validate, write-only, never revealed) and model (catalog/refresh/assignment with `If-Match`) contracts on the unmerged `feat/scoped-operator-auth` branch, covered by unit suites plus an end-to-end contract receipt (`tests/gateway/test_milestone2_receipt.py`) proving the sentinel-key-never-escapes invariant; Flutter `/providers` screen, write-only credential sheet, model picker, and exact-route read-only `/v1/models` fallback land with matching widget tests; the read-only fallback passed on physical device `RFCX81EJPNN` against saved gateway `BlueBlack`. | On-device provider/model administration receipt (secret non-reveal verified on a compatible real gateway); hermes-agent branch merge. |
| 3. Discover, Skills, Tools, and MCP | `implementing` | `/tools` shows gateway-scoped, authorized installed skills with bounded searchable description/category metadata and enabled toolsets with unsupported/empty/failure separation; legacy scope-free catalogs remain compatible while declared scopes fail closed and a physical-device inventory receipt. | Discover, install/remove, toolset mutation, and MCP require authoritative scoped contracts. |
...
| 7. Chat and Sessions completion | `implementing` | Concurrent session-owned streams survive switching, route approvals and stop to the originating/selected run, retain live transcripts and voice completion, expose background streaming/failure status, and preserve attached streams across ordinary Android resume. Status-confirmed detached runs persist only bounded opaque handles in secure storage, survive process recreation, fail closed against duplicate submission, and release after exact terminal status so authoritative history can reload. Streaming, bounded reasoning events, tools, retry, queued follow-ups, attachment-safe transcripts, session lifecycle/search/export, TTS, and bounded server-reported input/output/total token usage are also implemented with focused tests and prior Android receipts. | Physical concurrent-run and process-death detached-run receipts; clarify parity; optional redacted notifications; resource handles/context folders; runtime server-command discovery beyond the implemented client-owned slash commands; cost/cache/rate-limit/context metadata; and remaining history parity. |
